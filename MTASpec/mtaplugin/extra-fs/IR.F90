...
!     ************************************************************
      ...
      DIMENSION AWTISOTOP(NP)
      DIMENSION FREQ(NP),P(NP,NP),SGINT(NP),SEV(NP),DISNC(NP,NP)
      DIMENSION SORTFREQ(NP), SORTINT(NP), SORTRAMINT(NP), SORTREDF(NP)
      DIMENSION SORTDISCC(NP,NP)
      ...
      call getarg( 4, flout )
      call getarg( 5, fl )
      call getarg( 6, logfile )
       
      read(fl,*) FSCALE

      index = SCAN(geomfile,".")
      INPUTPREFIX = geomfile(1:index-1)       
...
!     Checking for mass of isotopes from file
      AWT = 0.0

      INQUIRE(FILE=TRIM(INPUTPREFIX)//"-mass", EXIST=file_exists)
      IF ( file_exists ) THEN
      OPEN(unit=3,file=TRIM(INPUTPREFIX)//"-mass",iostat=ios)
      DO
           READ(3,*,IOSTAT=IOs) J, AWTISOTOP(1)
           AWT(J) = AWTISOTOP(1)
           IF (ios .LT. 0) GOTO 50
      END DO
50    CLOSE(3)
...
!     READING THE ATOMIC WEIGHTS OF THE ELEMENTS
      DO 12 j=1,NATM
            IF (AWT(J).LE.0.0) AWT(j)= getAtomicMass( getAtomicNumber ( sym(J) ) )
...
!***********************************************************************
      ...
      kk = 0
      DO i = 1, N
        FREQ(i) = FSCALE*0.99963*0.05308837*(DSQRT(0.9375829*abs(D(i))))*(10**5)
        IF (D(i).lt.0.0) THEN
         kk = kk + 1
...
!     SYSTEM TO NORMAL COORDINATE SYSTEM
      ...
      !CALL SORTFRQNECIES(N,FREQ,FINT, SORTFREQ,SORTINT)
      !CALL SORTFRQNECIES(N,FREQ,FINT,REDF,RAMINT,DISCC,SORTFREQ,SORTINT,SORTREDF,SORTRAMINT,SORTDISCC)

      open(unit=2,file=flout(1:len_trim(flout)) ,iostat=ios)
      WRITE(2,808) (FREQ(i), FINT(i),I=7,N)
      CLOSE(2)
...
!****************************************************************************
        ...
        !CALL SORTFRQNECIES(N,FREQ,FINT, SORTFREQ,SORTINT)
        SUBROUTINE SORTFRQNECIES(N,A,B,C,D,E,FREQ,FINT,REDF,RAMINT,DISCC )
           IMPLICIT DOUBLE PRECISION(A-H,O-Z)
           PARAMETER (MAX_SIZE = 1000)
           DIMENSION A(MAX_SIZE),B(MAX_SIZE),FREQ(MAX_SIZE),FINT(MAX_SIZE)
           DIMENSION C(MAX_SIZE),D(MAX_SIZE),REDF(MAX_SIZE),RAMINT(MAX_SIZE)
           DIMENSION DISCC(MAX_SIZE,MAX_SIZE),E(MAX_SIZE,MAX_SIZE)
           ...
           FREQ = A; FINT = B; REDF = C; RAMINT = D; DISCC = E
...
! --------------------------------------------------------------------
           ...
           SUBROUTINE  Sort(x, Size,y)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      REAL*8,DIMENSION(1:), INTENT(INOUT) :: x,y
      ...
      END DO
           ...
           SUBROUTINE  Swap2D(a, b)
              IMPLICIT DOUBLE PRECISION(A-H,O-Z)
              REAL*8,DIMENSION(1:), INTENT(INOUT) :: a,b
              REAL*8,DIMENSION(1000) :: Temp
           ...
           SUBROUTINE  Sort2D(x, Size,y)
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      REAL*8,DIMENSION(1:), INTENT(INOUT) :: x
      REAL*8,DIMENSION(1:,1:), INTENT(INOUT) :: y 
      REAL*8,DIMENSION(1000,1000) :: z
      INTEGER, INTENT(IN)                   :: Size
      INTEGER                               :: i , Location
      !REAL*8                                :: Location
      ...
      DO i = 1, Size-1                  ! except for the last
         Location = FindMinimum(x, i, Size)     ! find min from this to last
         CALL  Swap(x(i), x(Location))  ! swap this and the minimum
         CALL  Swap2D(y(i,:), y(Location,:))  ! swap this and the minimum
